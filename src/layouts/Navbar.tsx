...
// Scroll to anchor — works across pages
  ...
  return (
    <div ref={ref} style={{ position: "relative", userSelect: "none" }}>
      <button className="lang-btn" onClick={() => setOpen((o) => !o)}>
        🌐 {current.nativeLabel}
        <span style={{ fontSize: ".55rem", opacity: 0.5 }}>
          {open ? "▲" : "▼"}
        </span>
      </button>
      {open && (
...
const Navbar: React.FC = () => {
  ...
  const links: { key: string; page?: string; anchor?: string }[] = [
    { key: "nav.home", page: "/" },
    { key: "nav.about", page: "/", anchor: "about" },
    { key: "nav.products", page: "/products" },
    { key: "nav.services", page: "/", anchor: "services" },
    { key: "nav.contact", page: "/", anchor: "contact" },
  ];
  ...
  const isActive = (page?: string, anchor?: string) => {
    if (anchor) return false; // anchor links don't stay "active"
        ...
        <div className="nav-inner">
          ...
          <ul className="nav-links">
            {links.map((l) => (
              <li key={l.key}>
                <button
                  className={`nav-link${isActive(l.page, l.anchor) ? " active" : ""}`}
                  onClick={() => handleNav(l.page, l.anchor)}
                >
                  {t(l.key)}
                </button>
              </li>
          ...
          <div className="nav-right">
            <div 
            // className="nav-cta-mobile-hide"
            >
              <LanguageSwitcher />
            </div>
            <button
              className="btn-red nav-cta-mobile-hide"
              style={{ padding: ".5rem 1.3rem", fontSize: ".75rem" }}
