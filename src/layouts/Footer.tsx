        ...
        <div className="footer-grid">
          <div>
            <div className="footer-logo-wrap">
              <img src={LOGO_URI} alt="COMES India" style={{ height: 44, width: 'auto', objectFit: 'contain', display: 'block' }} />
            </div>
            <p className="footer-desc">{t('ft.desc')}</p>
            <WhatsAppButton message={WHATSAPP_GENERAL} label="WhatsApp Us" />
          </div>
          <div>
            <div className="footer-h">{t('ft.ql')}</div>
            {[['nav.home','/'],['nav.about','/#about'],['nav.products','/products'],['nav.services','/#services'],['nav.contact','/#contact']].map(([k,p]) => (
              <Link key={k} to={p} className="footer-link">{t(k)}</Link>
            ))}
          </div>
          <div>
            <div className="footer-h">{t('ft.pr')}</div>
            {[['HTS-6 V14','hts-6-v14'],['RG6','rg6'],['STORM5 V14','storm5-v14']].map(([name,slug]) => (
              <Link key={slug} to={'/products/'+slug} className="footer-link">{name}</Link>
            ))}
            <div style={{ marginTop: '1.5rem', color: '#555', fontSize: '.82rem', lineHeight: 1.8 }}>
              <div style={{ color: 'var(--red)', fontFamily: 'var(--font-mono)', fontSize: '.58rem', letterSpacing: '.22em', marginBottom: '.4rem' }}>CONTACT</div>
              <div>+91-83066 65561</div>
              <div>info@comesindia.com</div>
              <div style={{ marginTop: '.3rem', fontSize: '.76rem', opacity: .7 }}>Mon–Sat 10AM–6PM</div>
            </div>
          </div>
        </div>
